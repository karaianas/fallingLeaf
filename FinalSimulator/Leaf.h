...
public:
	...
	Leaf(double len, double posX = 0, double posY = 0, double posZ = 0, double angT = 0)
	{
		...
		// Exp 2-1: total area AND aspect ratio changes
		//w = 1.0f;

		// Exp 2-2: total area changes but NOT aspect ratio
		//w = l * 10.0f;

		// Exp 3: aspect ratio changes but NOT total area
		w = 0.02 / len;
	...
	void draw_leaf3(int rgb)
	{
		...
		glTranslated(x, y, z);
		glRotated(theta, 0, 0, 1);
		//glScaled(100 * l, 0.5, 100 * w);
		glScaled(10 * l, 0.5, 10 * w);
		//glScaled(5 * l, 0.5, 5 * w); // (0.5, 10 * l, 10 * w); //glScaled(0.5, 10 * l, 10 * w);// 0.5, 5, 10
		glutSolidCube(1);
		glPushMatrix();
